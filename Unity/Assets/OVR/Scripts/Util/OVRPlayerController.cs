...
public class OVRPlayerController : MonoBehaviour
{
	...
	public virtual void UpdateMovement()
	{
		...
		if (MobileListener.Instance.IsTouching){
			...
			float leftTouchAxisY = touchPos.y * VRController.Instance.AxisScaleFactor;
			float leftTouchAxisX = touchPos.x * VRController.Instance.AxisScaleFactor;
			
			euler.y +=  MobileListener.Instance.RollSinceTouchStart * rotateInfluence;
					
			if(leftTouchAxisY > 0.0f)
				MoveThrottle += ort * (leftTouchAxisY * moveInfluence * Vector3.forward);
			
			if(leftTouchAxisY < 0.0f)
				MoveThrottle += ort * (Mathf.Abs(leftTouchAxisY) * moveInfluence * BackAndSideDampen * Vector3.back);
			
			if(leftTouchAxisX < 0.0f)
				MoveThrottle += ort * (Mathf.Abs(leftTouchAxisX) * moveInfluence * BackAndSideDampen * Vector3.left);
			
			if(leftTouchAxisX > 0.0f)
				MoveThrottle += ort * (leftTouchAxisX * moveInfluence * BackAndSideDampen * Vector3.right);
